# 1 "C:\\Users\\alexr\\Documents\\Arduino\\watch_20231203\\watch_20231203.ino"
# 2 "C:\\Users\\alexr\\Documents\\Arduino\\watch_20231203\\watch_20231203.ino" 2
# 3 "C:\\Users\\alexr\\Documents\\Arduino\\watch_20231203\\watch_20231203.ino" 2
...
Adafruit_SSD1306 display(128 /* OLED display width*/, 64 /* OLED display height*/, &Wire, -1);
...
void setup() {
  ...
  display.begin(0x02 /*|< Gen. display voltage from 3.3V*/, 0x3C); // 0x3C is the I2C address
 ...
 Serial.begin(115200);
  ...
  pinMode(button01, 0x05);
  pinMode(button02, 0x05);
 ...
 // Handle program selection
    if (digitalRead(button02) == 0x0) {
        ...
        if (digitalRead(button02) == 0x0) {
            ...
            while (digitalRead(button02) == 0x0); // Wait for button release
        }
    }

    // Check if the button is pressed
    if (digitalRead(button01) == 0x0) {
        // Debouncing delay
        delay(50);
        ...
        if (digitalRead(button01) == 0x0) {
            ...
            while (digitalRead(button01) == 0x0); // Wait for the button to be released
        }
    }
...
void setMotorSpeed(int speed, int direction) {
    // Set motor direction
    digitalWrite(motor1Pin1, direction == 0 ? 0x0 : 0x1);
    digitalWrite(motor1Pin2, direction == 0 ? 0x1 : 0x0);
...
void stopMotor() {
    ...
    decelerateToMinSpeed();
    // Stop motor
    digitalWrite(motor1Pin1, 0x0);
    digitalWrite(motor1Pin2, 0x0);
